...
const QUICK_COMMANDS: QuickCommand[] = [
  ...
  },
  {
    ...
    description: 'Install the app and CDK dependencies from the repo root',
  ...
  },
  {
    ...
    command:
      "Write-Host 'Specify distribution ID: aws cloudfront list-invalidations --distribution-id YOUR_DIST_ID'",
  ...
  },
  {
    ...
    command:
      'aws cloudformation describe-stacks --stack-name CDKToolkit --query "Stacks[0].{Status:StackStatus,Created:CreationTime}" --output table',
  ...
  // Cost & Billing Commands
  {
    ...
    command:
      'aws ce get-cost-and-usage --time-period Start=$(Get-Date -Day 1 -Format \'yyyy-MM-dd\'),End=$(Get-Date -Format \'yyyy-MM-dd\') --granularity MONTHLY --metrics BlendedCost --filter \'{"Dimensions":{"Key":"SERVICE","Values":["Amazon Simple Storage Service"]}}\' --output table',
  ...
  },
  {
    ...
    command:
      'aws ce get-cost-and-usage --time-period Start=$(Get-Date -Day 1 -Format \'yyyy-MM-dd\'),End=$(Get-Date -Format \'yyyy-MM-dd\') --granularity MONTHLY --metrics BlendedCost --filter \'{"Dimensions":{"Key":"SERVICE","Values":["Amazon CloudFront"]}}\' --output table',
    icon: <BarChart3 className="size-4" />,
    category: 'Cost & Billing',
  },
  ...
  },
  {
    ...
    command:
      'Write-Host \'=== Environment Check ===\'; Write-Host "Node: $(node --version)"; Write-Host "NPM: $(npm --version)"; Write-Host "CDK: $(npx cdk --version 2>&1)"; Write-Host "AWS CLI: $(aws --version 2>&1)"; Write-Host "AWS Account: $(aws sts get-caller-identity --query Account --output text 2>&1)"',
                ...
                {lastError ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
                    ...
                    <p className="text-xs mt-1">
                      Run a command first — if it fails, the error will appear here for AI analysis.
                    </p>
                  </div>
                )}
              </TabsContent>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-end gap-3">
              <div className="flex-1 space-y-1.5">
                <Label htmlFor="custom-cmd" className="text-xs">
                  Command
                </Label>
                <Input
                  id="custom-cmd"
              ...
              {saveMode && (
                ...
                >
                  <div className="flex items-end gap-3 pt-2">
                    <div className="flex-1 space-y-1.5">
                      <Label htmlFor="save-label" className="text-xs">
                        Name
                      </Label>
                      <Input
                        id="save-label"
                    ...
                    <div className="flex-1 space-y-1.5">
                      <Label htmlFor="save-desc" className="text-xs">
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1">
                            <Badge
                              variant="outline"
                              className={`text-[10px] shrink-0 ${CATEGORY_COLORS['Custom']}`}
                            >
                              Saved
                            </Badge>
                          </div>
  ...
  return (
    <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}>
      <Card className="h-full">
        <CardContent className="flex items-start gap-3 ">
          <div
            className={`rounded-md p-2 ${CATEGORY_COLORS[cmd.category] || CATEGORY_COLORS['Custom']}`}
          >
            {cmd.icon}
          </div>
          <div className="flex-1 min-w-0 space-y-1">
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge
                variant="outline"
                className={`text-[10px] shrink-0 ${CATEGORY_COLORS[cmd.category] || ''}`}
              >
                {cmd.category}
              </Badge>
            </div>
