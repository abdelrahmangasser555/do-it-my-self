...
export function diagnoseBootstrapError(errorOutput: string): ErrorDiagnosis {
  ...
  if (
    ...
    return {
      ...
      description: 'Your AWS IAM user does not have the required permissions to run CDK bootstrap.',
      fixSteps: [
  ...
  if (lower.includes('region') && (lower.includes('not enabled') || lower.includes('opt-in'))) {
    return {
      ...
      docsUrl: 'https://docs.aws.amazon.com/accounts/latest/reference/manage-acct-regions.html',
    };
  }
  ...
  return {
    ...
    description: 'CDK bootstrap encountered an error. Review the output below for details.',
    fixSteps: [
...
export const SETUP_RESOURCES = {
  ...
  nodejs: {
    ...
    videoSearch: 'https://www.youtube.com/results?search_query=install+nodejs+nvm+tutorial',
  },
};
