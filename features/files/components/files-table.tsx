      ...
      <TableBody>
        {files.map((file) => (
          <TableRow key={file.id}>
            <TableCell className="max-w-62.5 truncate font-mono text-xs">
              {file.objectKey}
            </TableCell>
            <TableCell>
  ...
  const filteredFiles = useMemo(() => {
    return files.filter((f) => {
      ...
      if (search) {
        ...
        const matchesName = getFileName(f.key).toLowerCase().includes(q);
        const matchesKey = f.key.toLowerCase().includes(q);
        if (!matchesName && !matchesKey) return false;
      }
      // Date range filter
      if (dateFrom && f.lastModified) {
        const mod = new Date(f.lastModified);
        if (mod < dateFrom) return false;
      }
      if (dateTo && f.lastModified) {
        const endOfDay = new Date(dateTo);
        endOfDay.setHours(23, 59, 59, 999);
        const mod = new Date(f.lastModified);
        if (mod > endOfDay) return false;
      }
      return true;
    });
