...
export async function POST(request: NextRequest) {
  try {
    ...
    const bucket: Bucket = {
      ...
      config: {
        // Default settings
        access: parsed.access ?? 'private',
        maxFileSizeMB: parsed.maxFileSizeMB ?? 100,
        strictFileSizeLimit: parsed.strictFileSizeLimit ?? false,
        allowedFileTypes: parsed.allowedFileTypes ?? 'any',
        autoDelete: parsed.autoDelete ?? false,
        autoDeleteDays: parsed.autoDeleteDays,

        // Advanced - Security
        signedUrlExpiration: parsed.signedUrlExpiration ?? 3600,
        corsOrigins: parsed.corsOrigins ?? ['*'],
        corsMethods: parsed.corsMethods ?? ['GET', 'PUT', 'POST'],

        // Advanced - Storage
        versioning: parsed.versioning ?? false,
        lifecycleTransitionDays: parsed.lifecycleTransitionDays,
        lifecycleDeleteIncompleteUploads: parsed.lifecycleDeleteIncompleteUploads ?? false,

        // Advanced - Performance
        enableCDN: parsed.enableCDN ?? true,
        cacheControl: parsed.cacheControl ?? 'public, max-age=31536000',

        // Advanced - Upload
        multipartThresholdMB: parsed.multipartThresholdMB ?? 100,
        maxConcurrency: parsed.maxConcurrency ?? 4,
        retryCount: parsed.retryCount ?? 3,

        // Advanced - Encryption
        encryptionType: parsed.encryptionType ?? 'S3',
        kmsKeyId: parsed.kmsKeyId,

        // Advanced - Monitoring
        enableAccessLogs: parsed.enableAccessLogs ?? false,
        enableMetrics: parsed.enableMetrics ?? false,

        // Advanced - Cost
        monthlyBudgetAlertUSD: parsed.monthlyBudgetAlertUSD,
  ...
  const stream = new ReadableStream({
    async start(controller) {
      const send = (step: string, status: string, error?: string) => {
        controller.enqueue(encoder.encode(JSON.stringify({ step, status, error }) + '\n'));
      };
      ...
      try {
        ...
        try {
          ...
          send(
            'cloudfront',
            'error',
            e instanceof Error ? e.message : 'Failed to delete CloudFront',
          );
        }
